...
int drawGUIMenu(std::string& srcPath, std::string& outPath) noexcept {
    struct winsize size;
    int height, width;
    ...
    int selectedChoice = 0;  // 0: 源文件, 1:输出文件， 2: 编译, 3: 退出
    bool running = true;
    ...
    while (running) {
        ...
        // 使用高亮显示当前选中的菜单项
        wprintf(L"\033[%d;%dH%ls 源文件 (F)", startY + 2, startX, (selectedChoice == 0 ? L"👉" : L"  "));
        wprintf(L"\033[%d;%dH%ls 输出文件 (O)", startY + 3, startX, (selectedChoice == 1 ? L"👉" : L"  "));
        wprintf(L"\033[%d;%dH%ls 编译 (C)", startY + 4, startX, (selectedChoice == 2 ? L"👉" : L"  "));
        wprintf(L"\033[%d;%dH%ls 退出 (Q)", startY + 5, startX, (selectedChoice == 3 ? L"👉" : L"  "));
        ...
        if (inputKey == 'w' || inputKey == 'W') {
            ...
            if (selectedChoice == 0) {
                wprintf(L"\033[%d;%dH请输入源文件路径: ", startY + 5, startX);
                ...
                wprintf(L"\033[%d;%dH请输入输出文件路径: ", startY + 5, startX);
                wprintf(L"\033[?25h");  // 恢复光标以供输入
                char ch = '\0';
                outPath = "";
                while ((ch = getchar()) != '\n' && ch != '\r') {
